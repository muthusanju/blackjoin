...
router.get('/cron',async (req,res) =>{
    ...
    var userdata11 = await db.AsyncFind('users',{}, {},{});

each(userdata11,async function (items,next) {

	var user_id   =  items._id;
	var crondate  =  items.createdAt;

    const userresult = await db.AsyncFind('users',{'_id':ObjectId(user_id),'createdAt':new Date(crondate)});
      if (userresult) {
      	var amount = userresult[0].amount/1000000;
        var admin_id=userresult[0]._id;
